...
final class CParser(AST) : Parser!AST
{
        ...
        while (1)
        {
            ...
            case TOK.dot:
                ...
                if (token.value == TOK.identifier)
                {
                    ...
                    e = new AST.DotIdExp(loc, e, makeIdentifierAtLoc(id, token.loc));
                    break;
                }
            ...
            case TOK.arrow:
                ...
                {
                    Identifier id = token.ident;
                    auto die = new AST.DotIdExp(loc, e, makeIdentifierAtLoc(id, token.loc));
                    die.arrow = true;
                    e = die;
                    break;
                }
    ...
    private AST.Expression cparseUnaryExp()
    {
        ...
        case TOK._Alignof:
        {
            ...
            check(TOK.rightParenthesis);
            e = new AST.TypeExp(loc, t);
            e = new AST.DotIdExp(loc, e, makeIdentifierAtLoc(Id.__xalignof, loc));
            break;
        }
            ...
            switch (token.value)
            {
                ...
                case TOK._Alignas:
                {
                    ...
                    if (isTypeName(tk))  // _Alignas ( type-name )
                    {
                        ...
                        exp = new AST.DotIdExp(loc, e, makeIdentifierAtLoc(Id.__xalignof));
                    }
    ...
    AST.ParameterList cparseParameterList()
    {
        ...
        while (1)
        {
            ...
            if (specifier.mod & MOD.xconst)
                t = toConst(t);
            auto param = new AST.Parameter(STC.parameter, t, makeIdentifierAtLoc(id), null, null);
    ...
    private void addBuiltinDeclarations()
    {
        void genBuiltinFunc(Identifier id, AST.VarArg va)
        {
            ...
            auto parameters = new AST.Parameters();
            parameters.push(new AST.Parameter(STC.parameter | STC.ref_, tva_list));
