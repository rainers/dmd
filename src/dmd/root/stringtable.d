...
private:
nothrow:
    StringEntry.vptr_t allocValue(const(char)* s, size_t length, void* ptrvalue)
    {
        version(GC)
        {
            auto vptr = mem.xmalloc(StringValue.sizeof + length + 1);
    ...
    StringValue* getValue(StringEntry.vptr_t vptr) pure
    {
        version(GC)
            return cast(StringValue*) vptr;
        else
        {
            if (!vptr)
                return null;
