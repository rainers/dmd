...
private extern (C++) final class StatementSemanticVisitor : Visitor
{
        ...
        case Tsarray:
            {
                ...
                s.saveOriginal(fs);                if (auto ls = checkLabeledLoop(sc, fs))   // https://issues.dlang.org/show_bug.cgi?id=15450
                                                          // don't use sc2
                    ls.gotoTarget = s;
                return retStmt(s);
            }
                                                     ...
                                                     Scope* sc2, Expression flde)
    {
         ...
         */
        Expression ec;
        ec = new DotIdExp(fs.loc, fs.aggr, makeIdentifierAtLoc(sapply.ident));
    ...
    override void visit(SynchronizedStatement ss)
    {
        ...
        else
        {
            ...
            enterArgs.push(new Parameter(0, t.pointerTo()));
            ...
            exitArgs.push(new Parameter(0, t));
