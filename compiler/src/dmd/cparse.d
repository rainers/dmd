...
final class CParser(AST) : Parser!AST
{
        ...
        while (1)
        {
            if (token.value == TOK.endOfFile)
            {
                ...
                if (importBuiltins)
                {
                     ...
                     */
                    auto s = new AST.Import(Loc.initial, null, Id.importc_builtins, makeIdentifierAtLoc(null), false);
                    wrap.push(s);
                }
    ...
    private AST.Expression cparseSizeofOrAlignofExp()
    {
        ...
        return new AST.DotIdExp(loc, e, makeIdentifierAtLoc(id, loc));
    }
    ...
    private AST.Expression cparseAlignasSpecifier(LVL level)
    {
        ...
        if (startsTypeName(&token))
        {
            ...
            exp = new AST.DotIdExp(loc, e, makeIdentifierAtLoc(Id.__xalignof, loc));
        }
        else
        {
            // _Alignas ( constant-expression )
    ...
    AST.ParameterList cparseParameterList()
    {
        ...
        while (1)
        {
            ...
            auto param = new AST.Parameter(id.name ? id.loc : typeLoc,
                                           ...
                                           t, makeIdentifierAtLoc(id.name), null, null, null);
                    ...
                    switch (token.value)
                    {
                        ...
                        Lwhile:
                            while (1)
                            {
                                ...
                                if (token.value != TOK.identifier)
                                    break Lswitch;
                                auto param = new AST.Parameter(token.loc, STC.none, null, makeIdentifierAtLoc(token.ident, token.loc), null, null, null);
                                parameters.push(param);
                                nextToken();
                                if (token.value == TOK.comma)
                                {
