...
extern (C++) struct Scope
{
    ...
    extern (C++) Dsymbol search(Loc loc, Identifier ident, out Dsymbol pscopesym, SearchOptFlags flags = SearchOpt.all)
    {
        ...
        Dsymbol checkAliasThis(AggregateDeclaration ad, IdentifierAtLoc ident, SearchOptFlags flags, Expression* exp)
        {
            import dmd.mtype;
            if (!ad || !ad.aliasthis)
                return null;
        ...
        Dsymbol searchScopes(SearchOptFlags flags)
        {
            ...
            NotFound:
                if (sc.previews.fixAliasThis)
                {
                    ...
                    if (Dsymbol aliasSym = checkAliasThis(sc.scopesym.isAggregateDeclaration(), makeIdentifierAtLoc(ident, loc), flags, &exp))
                    {
