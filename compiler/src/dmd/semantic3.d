            ...
            if (const nparams = f.parameterList.length)
            {
                ...
                foreach (i, fparam; f.parameterList)
                {
                    ...
                    if (!id)
                    {
                         ...
                         */
                        fparam.ident = makeIdentifierAtLoc(id = Identifier.generateId("__param_", i));
                        stc |= STC.temp;
                    }
                    Type vtype = fparam.type;
                    auto v = new VarDeclaration(identLoc(fparam.loc, fparam.ident), vtype, id, null);
            ...
            if (funcdecl.fbody)
            {
                ...
                if (funcdecl.inferRetType)
                {
                    ...
                    if (f.checkRetType(funcdecl.loc))
                        funcdecl.fbody = ErrorStatement.get(funcdecl.fbody);
                    else
