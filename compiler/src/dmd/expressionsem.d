...
private Module loadStdMath()
{
    ...
    __gshared IdentifierAtLoc[1] stdID;
    if (!impStdMath)
    {
        stdID[0] = makeIdentifierAtLoc(Id.std);
        auto s = new Import(Loc.initial, stdID[], Id.math, makeIdentifierAtLoc(null), false);
...
Expression semanticY(DotIdExp exp, Scope* sc, int flag)
{
    ...
    else
    {
        if (exp.e1.isTypeExp() || exp.e1.isTemplateExp())
            flag = 0;
        Expression e = exp.e1.type.dotExp(sc, exp.e1, exp, flag | (exp.noderef ? DotExpFlag.noDeref : 0));
        if (e)
        {
            e = e.expressionSemantic(sc);
        }
