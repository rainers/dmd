...
Expression resolveOpDollar(Scope* sc, ArrayExp ae, IntervalExp ie, ref Expression pe0)
{
    ...
    return errors ? ErrorExp.get(ae) : ae;
}
...
{
    ErrorExp errorReturn()
    {
        version (LanguageServer)
            return ErrorExp.get(new CommaExp(Loc.initial, e1, e2));
        else
            return ErrorExp.get(null);
    }
...
private extern (C++) final class ExpressionSemanticVisitor : Visitor
{
    ...
    override void visit(InterpExp e)
    {
        ...
        Expression makeNonTemplateItem(Identifier which) {
            ...
            id = new DotIdExp(e.loc, id, makeIdentifierAtLoc(which));
    ...
    */
    ...
    Lagain:
        ...
        if (t1)
        {
            ...
            L1:
                ...
                Expression e = new DotIdExp(exp.loc, exp.e1, makeIdentifierAtLoc(Id.opCall));
                e = new CallExp(exp.loc, e, exp.arguments, exp.names);
                e = e.expressionSemantic(sc);
                result = e;
                return;
            ...
            case TOK.parameters:
                {
                    ...
                    foreach (i, arg; tded.isTypeFunction().parameterList)
                    {
                        ...
                        if (e.tok2 == TOK.parameters && arg.defaultArg && arg.defaultArg.op == EXP.error)
                            return setError();
                        args.push(new Parameter(arg.loc, arg.storageClass, arg.type,  makeIdentifierAtLoc((e.tok2 == TOK.parameters) ? arg.ident : null), (e.tok2 == TOK.parameters) ? arg.defaultArg : null, arg.userAttribDecl, (e.tok2 == TOK.parameters) ? arg.unpack : null));
                    }
                    tded = new TypeTuple(args);
                    break;
                }
    ...
    override void visit(AssertExp exp)
    {
        ...
        // no message - use assert expression as msg
        {
            ...
            */
            ...
            auto assertFail = new DotIdExp(loc, __assertFail, makeIdentifierAtLoc(Id.object));
    ...
    override void visit(AssignExp exp)
    {
        ...
        if (auto ae = exp.e1.isArrayExp())
        {
            ...
            while (true)
            {
                ...
                if (search_function(ad, Id.opIndexAssign))
                {
                    ...
                    a.insert(0, exp.e2);
                    res = new DotIdExp(exp.loc, ae.e1, makeIdentifierAtLoc(Id.opIndexAssign));
            ...
            Lfallback:
                if (maybeSlice && search_function(ad, Id.opSliceAssign))
                {
                    ...
                    res = new DotIdExp(exp.loc, ae.e1, makeIdentifierAtLoc(Id.opSliceAssign));
                    res = new CallExp(exp.loc, res, a);
                    res = res.expressionSemantic(sc);
                    return setResult(Expression.combine(e0, res));
                }
            ...
            if (exp.op == EXP.construct)
            {
                ...
                else if (!e2x.implicitConvTo(t1))
                {
                    ...
                    if (search_function(sd, Id.opCall))
                    {
                         ...
                         */
                        e2x = typeDotIdExp(e2x.loc, e1x.type, makeIdentifierAtLoc(Id.opCall));
        ...
        if (auto ae = res.isConstructExp())
        {
            ...
            if (lowerToArrayCtor || lowerToArraySetCtor)
            {
                ...
                lowering = new DotIdExp(ae.loc, lowering, makeIdentifierAtLoc(Id.object));
                lowering = new DotIdExp(ae.loc, lowering, makeIdentifierAtLoc(func));
    ...
    private Expression lowerArrayAssign(AssignExp ae, bool fromCommaExp = false)
    {
        ...
        id = new DotIdExp(ae.loc, id, makeIdentifierAtLoc(Id.object));
        id = new DotIdExp(ae.loc, id, makeIdentifierAtLoc(func));
    ...
    Laliasthis:
        ...
        if ((exp.op == EXP.concatenateAssign || exp.op == EXP.concatenateElemAssign) && sc.needsCodegen())
        {
            ...
            if (exp.op == EXP.concatenateAssign)
            {
                ...
                Expression id = new IdentifierExp(exp.loc, Id.empty);
                id = new DotIdExp(exp.loc, id, makeIdentifierAtLoc(Id.object));
                id = new DotIdExp(exp.loc, id, makeIdentifierAtLoc(hook));
    ...
    override void visit(PowExp exp)
    {
        ...
        if (exp.e2.op == EXP.float64 && exp.e2.toReal() == CTFloat.half)
        {
            ...
            Expression id = new IdentifierExp(exp.loc, Id.empty);
            id = new DotIdExp(exp.loc, id, makeIdentifierAtLoc(Id.object));
            id = new DotIdExp(exp.loc, id, makeIdentifierAtLoc(Id._d_sqrt));
        ...
        else
        {
            ...
            Expression id = new IdentifierExp(exp.loc, Id.empty);
            id = new DotIdExp(exp.loc, id, makeIdentifierAtLoc(Id.object));
            id = new DotIdExp(exp.loc, id, makeIdentifierAtLoc(Id._d_pow));
    ...
    */
    ...
    override void visit(EqualExp exp)
    {
        ...
        if (isArrayComparison && !shouldUseMemcmp(t1, t2, sc))
        {
            ...
            {
                Expression exp_l1 = new DotIdExp(exp.e1.loc, exp.e1, makeIdentifierAtLoc(Id.length));
                Expression exp_l2 = new DotIdExp(exp.e2.loc, exp.e2, makeIdentifierAtLoc(Id.length));
            ...
            Expression lowering = new IdentifierExp(exp.loc, Id.empty);
            lowering = new DotIdExp(exp.loc, lowering, makeIdentifierAtLoc(Id.object));
            lowering = new DotIdExp(exp.loc, lowering, makeIdentifierAtLoc(hook));
...
private Expression dotIdSemanticPropX(DotIdExp exp, Scope* sc)
{
    ...
    {
        // bypass checkPurity
        return exp.e1.type.dotExp(sc, exp.e1, exp, cast(DotExpFlag) (exp.noderef * DotExpFlag.noDeref));
    }
    ...
    if (auto te = exp.e1.isTupleExp())
    {
            ...
            exp.ident == Id.bitwidth)
        {
            ...
            foreach (i, e; (*te.exps)[])
            {
                (*exps)[i] = new DotIdExp(e.loc, e, makeIdentifierAtLoc(exp.ident, exp.loc));
            }
...
Expression dotIdSemanticProp(DotIdExp exp, Scope* sc, bool gag)
{
    ...
    if (auto ie = eright.isScopeExp()) // also used for template alias's
    {
        ...
        if (s)
        {
            ...
            if (OverDeclaration od = s.isOverDeclaration())
            {
                Expression e = new VarExp(exp.identloc, od, true);
                if (eleft)
                {
                    e = new CommaExp(exp.loc, eleft, e);
                    e.type = Type.tvoid; // ambiguous type?
            ...
            if (auto tup = s.isTupleDeclaration())
            {
                if (eleft)
                {
                    Expression e = new DotVarExp(exp.identloc, eleft, tup);
                    e = e.expressionSemantic(sc);
                    return e;
                }
              ...
              ))
    {
        ...
        return e.type.dotExp(sc, e, exp, newFlag);
    }
    ...
    else
    {
        ...
        Expression e = dotExp(exp.e1.type, sc, exp.e1, exp, flag);
        if (e)
        {
            e = e.expressionSemantic(sc);
        }
...
extern(D) void lowerArrayAggregate(StaticForeach sfe, Scope* sc)
{
    ...
    auto ael = new ArrayLengthExp(aggr.loc, aggr);
    sc = sc.startCTFE();
    Expression el = ael;
    el = el.expressionSemantic(sc);
    sc = sc.endCTFE();
    ...
    if (el.op != EXP.int64)
    {
        sfe.aggrfe.aggr = ErrorExp.get(ael);
        return;
    }
