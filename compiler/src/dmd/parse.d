...
class Parser(AST, Lexer = dmd.lexer.Lexer) : Lexer
{
  ...
  final:
     ...
     *  pidentLoc    = if !null, then set to location of the identifier
     */
    AST.Type parseType(Identifier* pident = null, AST.TemplateParameters** ptpl = null, Loc* pidentLoc = null)
    {
    ...
    Lwhile:
        ...
        else if (isDeclaration(&token, NeedDeclaratorId.must, TOK.assign, null))
        {
            ...
            check(TOK.assign);
            param = new AST.Parameter(aloc, storageClass, at, makeIdentifierAtLoc(ai, aloc), null, null);
        }
